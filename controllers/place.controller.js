  ...
  const findByCampusId = async (req, res, next) => {
    ...
    if (campusId) {
      try {
        ...
        const counts = {
          empty: countBy(mappedPlaces, p => p.peopleCount === 0 && p.remainingTime !== 0),
          quiet: countBy(mappedPlaces, p => p.noise === 1 && p.remainingTime !== 0),
          availableMoreThanOneHour: countBy(mappedPlaces, p => p.remainingTime > 60),
          withProjector: countBy(mappedPlaces, p => p.equipments.includes('projector') && p.remainingTime !== 0),
        };
        res.send({ counts, places: mappedPlaces });
      } catch (e) {
        res
          .status(404)
          .json({
